...
class AnchoredEllipse(AnchoredOffsetbox):
    def __init__(self, transform, width, height, angle, loc,
                 pad=0.5, borderpad=0.2, prop=None, frameon=True, color='blue'):
        '''
        ...
        self.ellipse = Ellipse(xy=(0, 0), width=width, height=height,
                               angle=angle, color='grey', ec='black',
                               fill=True, hatch='\\\\\\\\\\\\')
        self._box.add_artist(self.ellipse)
        super(AnchoredEllipse, self).__init__(loc, pad=pad, borderpad=borderpad,
                         child=self._box, prop=prop, frameon=frameon)



def PlotComp(mfitcomps, comp_color='blue', comp_lw=1.3,
             comp_labelling='textalloc', comp_label_pos=None):
    '''
    # Purpose:
    ...
    def circros(x, y, r):
        circ = mpl.patches.Circle(xy=(x,y), radius=r, fill=False, color=comp_color,
                                  linewidth=comp_lw, zorder=4, path_effects=[outline])
        ax=plt.gca()
        ax.add_artist(circ)
        plt.vlines(x, y+r, y-r, linestyle='-', color=comp_color, linewidth=comp_lw,
                   zorder=4, path_effects=[outline])
        plt.hlines(y, x+r, x-r, linestyle='-', color=comp_color, linewidth=comp_lw,
                   zorder=4, path_effects=[outline])

    def elcros(x, y, a, b, phi):
        el = mpl.patches.Ellipse(xy=(x,y), width=a, height=b, angle=phi,
                                 color=color, fill=False, hatch='\\',
                                 zorder=4, linewidth=comp_lw)
        ax=plt.gca()
        ax.add_artist(el)
        plt.vlines(x, y+b/2, y-b/2, linestyle='-', color=comp_color, linewidth=comp_lw,
                   zorder=4, path_effects=[outline])
        plt.hlines(y, x+a/2, x-a/2, linestyle='-', color=comp_color, linewidth=comp_lw,
                   zorder=4, path_effects=[outline])
    ...
    if comp_label_pos != None:
        ...
        for i, comp in enumerate(mfitcomps):
            plt.text(comp_label_pos[i][0], comp_label_pos[i][1],
                     str(comp.name), color=comp_color, size=15,
                     weight='bold')
            plt.plot([comp.ra, comp_label_pos[i][0]],
        ...
        for comp in mfitcomps:
            if comp.axratio == 1:
                if comp.major >= comp.dlim:
                    plt.text(comp.ra-0.5*comp.a, comp.dec+1.5*comp.a,
                             str(comp.name), color=comp_color, size=15,
                             weight='bold')
                else:
                    plt.text(comp.ra-0.3, comp.dec+0.3, str(comp.name),
                             color=comp_color, size=15, weight='bold')
            else:
                if comp.major >= comp.dlim/10:
                    plt.text(comp.ra-1.5*comp.a, comp.dec+1.5*comp.a,
                             str(comp.name), color=comp_color, size=15,
                             weight='bold') 
                else:
                    plt.text(comp.ra-0.3, comp.dec+0.3, str(comp.name),
...
def plot_image_comps(components,
                     IMAP_file,
                     rms,
                     plt_xlim,
                     plt_ylim,
    ...
    ydim = hdu0.header.cards['naxis2'][1]    # the dimensions of the map
        # in pixels
    delt = round(hdu0.header.cards['cdelt2'][1]*3600*1000,3)    #mas per
    ...
    # ellipse = mpl.patches.Ellipse(xy=( beam_xpos, beam_ypos ),
    ...
    if not any(x == None for x in (bmaj, bmin, beam_angle)):
        ae = AnchoredEllipse(ax.transData, width=bmin, height=bmaj,
                             angle=beam_angle_rot, loc='lower left',
                             pad=0.8, borderpad=0.3, frameon=False, prop=dict())
        ax.add_artist(ae)
    else:
        print('Warning: could not plot beam because no info was provided.')

    ### Annotate source name (old) ###
    # plt.annotate(srcname, xy=(srcname_xpos, srcname_ypos),
                 # color='black', fontsize=srcname_fontsize)

    ### Annotate source name (NEW, automatically adjusted to figure) ###
    at_1 = AnchoredText(srcname, loc='upper left',
                        pad=0.8, borderpad=0.2, frameon=False,
    ...
    at_2 = AnchoredText(epochname+'\n\n'+str(freq)+' GHz', loc='upper right',
                        pad=0.8, borderpad=0.2, frameon=False,
                        prop=dict(size=epochname_fontsize, color='black'))
    ax.add_artist(at_2)

    ### Annotate observing frequency (old) ###
    # plt.annotate(str(freq)+' GHz', xy=(freq_xpos, freq_ypos),
                 # color='black', fontsize=freq_fontsize)

    levels_listed = np.array(['{:0.2f}'.format(x) for x in Ilevs])    # list
      # total intensity contour levels
    ...
    ax = plt.gca()
    try:
        ax.contour(xp, yp, IMAP_data[0,0,:,:], levels=contours,
    ...
    plt.text(-0.01, 1.05,
        ...
        multialignment='left', fontsize=14, transform=ax.transAxes)
    
    if components != None:
        ### Plot modelfit components overlaid ###
        for i, comp in enumerate(components):
            # shift components by the same amount as the map (+1 pixel in RA due to
                # difmap not counting the first row of values
            if comp.is_shifted == False:
                components[i] = components[i].shift((shift_x+1)*delt, shift_y*delt)
...
def plot_grouped_by_freq(df, y_col, y_err_col, freq_tolerance=1E9, y_label='',
    ...
    if y_col == "SBR" or y_col == 'FR':
        print('\n')
        print('# Plotting all data for ' + y_col + ' #')
        ...
        if show == True:
            plt.show()
        plt.close()
        print(f"Saved combined {y_label} plot: {filename}")
