...
class Image_dataset:
    ...
    def __init__(self,
        ...
        # self.header = fits.get_header(self.fits_file_name)
        self.header = fits.open(self.fits_file_path)[0].header
        ...
        self.bmaj = fits.open(self.fits_file_path)[0].header['BMAJ']
        self.bmin = fits.open(self.fits_file_path)[0].header['BMIN']
        self.bpa = fits.open(self.fits_file_path)[0].header['BPA']
        self.ps = fits.open(self.fits_file_path)[0].header['CDELT2']*3.6E6
        ...
        self.rms_clean_fit = helpers.fit_image_comps(self.comps, IMAP_file=self.fits_file_path)
        
        for comp in self.comps:
            comp.calc_errors(difmap_path=self.difmap_path, weight=difmap_weight,
                             beam=[self.bmaj, self.bmin, self.bpa],
                             rms_=self.rms_clean_fit, gain_err=self.gain_err,
                             shift=self.shift)
    ...
    def get_freq(self):
        ...
        header = fits.open(self.fits_file_path)[0].header
...
class components:
    ...
    def __init__(self):
        ...
        self.flux_err = None
        self.Sp = None    # intensity of the pixel at the position of the
            # component
        self.Sp_err = None
        self.dist_err = None
        self.theta_err = None
    ...
    def calc_errors(self, difmap_path='/usr/local/difmap/difmap', comp_name=None,
                    weight=[0,-1], gain_err=None, beam=None, rms_=None, shift=None):
        if comp_name == None:
            comp_name = self.name
        if weight[0] > 0:
            uv_weight = 'u'
        ...
        if self.fits_file != '' and self.uvfits_file != '' and self.modelfit_file != '' and self.mfitid_file != '':
            ...
            if self.name == None:
                print('Error: cannot compute uncertainties as no components have been read yet.')
                return 0
            ...
            # sigma_t = np.sqrt((gain_err*S_t)**2 + rms**2)    # Other calculation (now
                # obsolete): calculate error just from image rms and gain error
            sigma_t = sigma_p*np.sqrt(1+(S_t**2/S_p**2))    # According to Frank
            # Schinzel's PhD thesis
            
            '''
            IMPORTANT: previously, the gain error has been used as below. However,
            when we calculate the flux ratio and surface brightness ratio later based
            on the total flux and its error, the gain error should affect all
            components in the image in the same way: thus, we can leave it out. For
            reporting the error on just the total flux density, the gain error should
            be considered.
            '''
            sigma_t = np.sqrt(sigma_t**2 + (gain_err*S_t)**2)    # Add gain error in
            ...
            if uv_weight == 'u' or uv_weight == 'U' or uv_weight == 'uniform':
                d_lim = 4./np.pi*np.sqrt(np.pi*np.log(2)*b_maj*b_min*np.log((SNR+1)/SNR))
            else:    # this is the condition for natural weight
                d_lim = 2./np.pi*np.sqrt(np.pi*np.log(2)*b_maj*b_min*np.log((SNR+1)/SNR))
            ...
            if fwhm >= d_lim:
                sigma_T_b_obs = np.sqrt(  (sigma_t*1E-26*c**2/(2*k_B*nu**2*fwhm_rad**2))**2
                                        + (-2*sigma_fwhm_rad*S_t*1E-26*c**2/(2*k_B*nu**2*fwhm_rad**3))**2
                                        )
            else:
                print('Component {0:s} is unresolved'.format(self.name))
                print('Calculated observed brightness temperature is a lower limit')
            ...
            T_b_obs = S_t*1E-26*c**2/(2*k_B*nu**2*fwhm_rad**2)
            sigma_T_b_obs = np.sqrt(  (sigma_S_t*1E-26*c**2/(2*k_B*nu**2*fwhm_rad**2))**2
                                    + (-2*sigma_fwhm_rad*S_t*1E-26*c**2/(2*k_B*nu**2*fwhm_rad**3))**2
                                    )
    ...
    def print_info(self):
        ...
        if self.dist_err != None:
            print(f'# Distance in polar coordinates: {self.dist:.3f} +/- {self.dist_err:.3f} mas')
        else:
            print(f'# Distance in polar coordinates: {self.dist:.3f} mas')
        if self.theta_err != None:
            print(f'# Angle in polar coordinates: {self.theta:.1f} +/- {self.theta_err:.1f} deg')
        else:
            print(f'# Angle in polar coordinates: {self.theta:.1f} deg')
        if self.major_err != None:
            print(f'# Major axis FWHM: {self.major:.5f} +/- {self.major_err:.5f} mas')
        else:
            print(f'# Major axis FWHM: {self.major:.5f} mas')
        if self.minor != None:
            if self.minor_err != None:
                print(f'# Minor axis FWHM: {self.minor:.5f} +/- {self.minor_err:.5f} mas')
            else:
                print(f'# Minor axis FWHM: {self.minor:.5f} mas')
        else:
            print(f'# Axis ratio: {self.axratio:.2f}')
        if self.phi_err != None:
            print(f'# Major/minor axis position angle: {self.phi:.1f} +/- {self.phi_err:.5f} deg')
        else:
...
class lens_candidate():
    ...
    def __init__(self,
        ...
        if len(fits_files) and len(uvf_files) and len(mfit_files) and len(mfitid_files) != 0:
            self.image_datasets = [Image_dataset(fits_file, uvf_file, mfit_file, mfitid_file, difmap_path=self.difmap_path) for (fits_file, uvf_file, mfit_file, mfitid_file) in zip(fits_files, uvf_files, mfit_files, mfitid_files)]
        else:
            self.image_datasets = []
        ...
        for image_dataset in self.image_datasets:
            ...
            if not any(abs(num - image_dataset.freq) <= freq_tolerance for num in self.freqs):
                self.freqs.append(image_dataset.freq)
            if not any(abs(num - image_dataset.mjd) <= date_tolerance for num in self.mjds):
                self.epochs.append(image_dataset.epoch)
                self.mjds.append(image_dataset.mjd)
    
    
    
    def read_all(self, in_dir='./', shift=[0,0], difmap_weight=[0,-1]):
        print('### READING DATA ###')
        ...
        if not len(fits_files) == len(uvf_files) == len(mfit_files):
            print('Error: non-matching number .fits, .uvfits or .mfit files in the specified folder. Data have not been read.')
            # print(fits_files)
            # print(uvf_files)
            # print(mfit_files)
            return 0
        ...
        self.image_datasets = self.image_datasets + [Image_dataset(
            fits_file,
            uvf_file,
            mfit_file,
            mfitid_file,
            ...
            ) for (fits_file, uvf_file, mfit_file, mfitid_file) in zip(fits_files, uvf_files, mfit_files, mfitid_files)]
    ...
    def add_image_dataset(self, fits_file='', uvf_file='', mfit_file='', mfitid_file='',
                          shift=[0,0], difmap_weight=[0,-1]):
        print('### ADDING IMAGE DATASET ###')
        if any(file == '' for file in [fits_file, uvf_file, mfit_file, mfitid_file]):
            print('Error: need to provide .fits, .uvfits, .mfit and .mfitid file!')
            return
