...
class Image_dataset:
    ...
    def __init__(self,
        ...
        if rms_method == 'image_fit':
            ...
            rms_use = helpers.get_rms(fits_file=self.fits_file_path,
...
class components:
    ...
    def calc_errors(self, difmap_path='/usr/local/difmap/difmap', comp_name=None,
        ...
        if self.fits_file != '' and self.uvfits_file != '' and self.modelfit_file != '' and self.mfitid_file != '':
            print('Calculate component errors using .uvfits, .fits, .mfit and .mfitid files.')
            if self.name == None:
...
class lens_candidate():
    ...
    def __init__(self,
        ...
        if len(fits_files) and len(uvf_files) and len(mfit_files) and len(mfitid_files) != 0:
            self.image_datasets = [
                Image_dataset(fits_file, uvf_file, mfit_file, mfitid_file, difmap_path=self.difmap_path, rms_method=self.rms_method)
    ...
    def read_all(self, in_dir='./', shift=[0,0], difmap_weight=[0,-1], rms_method=None):
        print('### READING DATA ###')
    ...
    def add_image_dataset(self, fits_file='', uvf_file='', mfit_file='', mfitid_file='',
                          shift=[0,0], difmap_weight=[0,-1], rms_method=None):
