...
def fit_image_comps(components, IMAP_file, output=False, outpath='./',
    ...
    IMAP_data = fits.getdata(IMAP_file)    # load up the fits image
    IMAP_data = np.squeeze(IMAP_data)    # remove redundant axes

    ### Extracting information from the fits header ###
    hdulist = fits.open(IMAP_file)
    header = fits.open(IMAP_file)[0].header
    hdu0 = hdulist[0]
    try:
        ...
        xdim = len(np.array(IMAP_data)[0])
        ydim = len(np.array(IMAP_data)[:,0])
    delt = round(hdu0.header.cards['cdelt2'][1]*3600*1000,3)    # mas per pixel
    ...
    try:
        popt, pcov = curve_fit(_gaussian, xdata, IMAP_data.ravel(), p0)
        ...
        popt, pcov = curve_fit(_gaussian, xdata, IMAP_data.ravel(), np.array(p0)*1.01)

    fit = np.zeros(IMAP_data.shape)
    for i in range(len(popt)//6):
        fit += gauss2d(X, Y, *popt[i*6:i*6+6])

    masked_data = IMAP_data - fit

    rms = np.sqrt(np.mean((masked_data)**2))
    ...
    if output == True:
        ...
        for i in range(0, xdim):
            for j in range(0, ydim):
                if IMAP_data[j,i] > IMAP_max:
                    IMAP_max = IMAP_data[j,i]
                    j_max = j
                    i_max = i
        ...
        IMAP_name = IMAP_file[0:-5].rsplit('/', 1)[-1]
        if outpath[-1] != '/':
            outpath = outpath + '/'
        
        hdu = fits.PrimaryHDU(fit, header=header)
        hdulist = fits.HDUList([hdu])
        hdulist.writeto(outpath + IMAP_name + '_fitted_gaussians.fits',
                        overwrite=True)

        hdu = fits.PrimaryHDU(masked_data, header=header)
        hdulist = fits.HDUList([hdu])
        hdulist.writeto(outpath + IMAP_name + '_fit_residuals.fits',
                        overwrite=True)
        
        ### Calculate contour levels ###
        firstcont = rms*4/IMAP_max*100    # percentage of the first cont set at 4-sigma
        ...
        ### Plot the image and the fit as overlaid contours ###
        fig = plt.figure(1)
        ax = fig.add_subplot(111)
        imap_data_plot = IMAP_data.copy()
        imap_data_plot[imap_data_plot<=0] = 1e-9
        
        fig = plt.figure()
        axIm = plt.imshow(imap_data_plot, origin='lower', cmap='plasma',
                          norm='log', extent=[xmin, xmax, ymin, ymax],
                          vmin=rms, vmax=IMAP_max)
        plt.contour(xp, (-1)*yp, fit, colors='w', levels=contours)
        plt.xlabel('Relative RA [mas]', fontsize=14)
        plt.ylabel('Relative DEC [mas]', fontsize=14)
        plt.savefig(outpath + IMAP_name + '_imap+gaussfit.png')
        plt.cla()
        plt.clf()
        plt.close(fig)
        plt.close('all')
...
def gauss2d(x, y, x0, y0, sigma_X, sigma_Y, theta, A):
    ...
    # if y0 < 0 or y0 > 2048:
        # return 1e9
    if sigma_X < 0 or sigma_X > 150:
        return 1e9
    if sigma_Y < 0 or sigma_Y > 150:
        return 1e9
...
def get_comp_peak_rms(fits_file, uvf_file, mfit_file, mfitid_file, comp_name='',
    ...
    if par_file != '' and os.path.exists('./'+par_file):
        ...
        components = rc.ReadComp(mfitid_file)    # read here the no_shift map
        S_p = []
        rms = []
        ...
        for comp in components:
            ...
            rms_ = np.std(resMAP_data[ int(round(ydim/2 + dec/ps_y, 0))  - int(rms_box/2)
                                      ...
                                      :int(round(xdim/2 - ra/ps_x, 0))-1 + int(rms_box/2)])
            rms.append(rms_)
    print('Peak flux densitiy after component subtraction [Jy/beam]:')
    print(S_p[0])
...
def calc_lens_params(components, outfile, freq, gain_err=0.0):
    ...
    if len(S_t_A) == 1 and len(S_t_B) == 1:
        ...
        sig_T_b_obs_A_max = sig_T_b_obs_A
        log_T_b_obs_A_max = log_T_b_obs_A
        log_sig_T_b_obs_A_max = sig_log_T_b_obs_A
...
def limit_formatter(x):
        """Format values, preserving >/<"""
        if pd.isna(x):
            return '-'
        x = str(x)
        # Detect if it has limit symbol
        if x.startswith(('$>$ ', '$<$ ')):
            try:
                num = float(x[4:])
                return f"{x[0:3]}{num:.2f}"
            except ValueError:
                return x  # just return as-is if cannot parse number
        elif x.startswith(('$>$', '$<$')):
            try:
                num = float(x[3:])
                return f"{x[0:2]}{num:.2f}"
            except ValueError:
                return x  # just return as-is if cannot parse number
        else:
            try:
